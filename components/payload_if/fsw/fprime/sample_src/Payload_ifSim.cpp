// ======================================================================
// \title  Payload_ifSim.cpp
// \author jstar
// \brief  cpp file for Payload_ifSim component implementation class
// ======================================================================

#include "payload_if_src/Payload_ifSim.hpp"
...
namespace Components {
  ...
  Payload_ifSim ::
    Payload_ifSim(const char *const compName) : Payload_ifSimComponentBase(compName)
{
    ...
    status = uart_close_port(&Payload_ifUart);

    HkTelemetryPkt.DeviceEnabled = PAYLOAD_IF_DEVICE_DISABLED;
  ...
  void Payload_ifSim :: NOOP_cmdHandler(FwOpcodeType opCode, U32 cmdSeq) {
    ...
    this->tlmWrite_ReportedComponentCount(Payload_ifHK.DeviceCounter);
    this->tlmWrite_DeviceConfig(Payload_ifHK.DeviceConfig);
    this->tlmWrite_DeviceStatus(Payload_ifHK.DeviceStatus);
    this->tlmWrite_DeviceEnabled(get_active_state(HkTelemetryPkt.DeviceEnabled));

    this->cmdResponse_out(opCode, cmdSeq, Fw::CmdResponse::OK);
  }

  void Payload_ifSim :: REQUEST_HOUSEKEEPING_cmdHandler(FwOpcodeType opCode, U32 cmdSeq) {
    
    if(HkTelemetryPkt.DeviceEnabled == PAYLOAD_IF_DEVICE_ENABLED)
    {
  ...
  void Payload_ifSim :: PAYLOAD_IF_SEQ_cmdHandler(FwOpcodeType opCode, U32 cmdSeq) {
  ...
  void Payload_ifSim :: ENABLE_cmdHandler(FwOpcodeType opCode, U32 cmdSeq) {

    if(HkTelemetryPkt.DeviceEnabled == PAYLOAD_IF_DEVICE_DISABLED)
    {
      ...
      Payload_ifUart.isOpen        = PORT_CLOSED;
      Payload_ifUart.baud          = PAYLOAD_IF_CFG_BAUDRATE_HZ;
      Payload_ifUart.access_option = uart_access_flag_RDWR;

      status = uart_init_port(&Payload_ifUart);
      if(status == OS_SUCCESS)
      {

        HkTelemetryPkt.DeviceEnabled = PAYLOAD_IF_DEVICE_ENABLED;
        HkTelemetryPkt.DeviceCount++;
        
        this->log_ACTIVITY_HI_TELEM("Successfully Enabled");  
        OS_printf("Payload_ifSim Enable Succeeded\n");  
      }
      else
      {
        HkTelemetryPkt.DeviceErrorCount++;
        this->log_ACTIVITY_HI_TELEM("Enable failed, failed to init UART port");  
        OS_printf("Payload_ifSim Enable Failed to init UART port\n");  
      }
    }
    else
    {
      ...
      OS_printf("Payload_ifSim Enable Failed, Already Enabled\n");
    }
  ...
  void Payload_ifSim :: DISABLE_cmdHandler(FwOpcodeType opCode, U32 cmdSeq) {
    ...
    if(HkTelemetryPkt.DeviceEnabled == PAYLOAD_IF_DEVICE_ENABLED)
    {
      ...
      if (status == OS_SUCCESS)
      {
        ...
        OS_printf("Payload_ifSim Disable Succeeded\n");
      }
      else
      {
        HkTelemetryPkt.DeviceErrorCount++;
        this->log_ACTIVITY_HI_TELEM("Disable Failed to close UART port");  
        OS_printf("Payload_ifSim Disable Failed to close UART port\n");
      }
    }
    else
    {
      ...
      OS_printf("Payload_ifSim Disable Failed, device already disabled\n");
    }
  ...
  void Payload_ifSim :: RESET_COUNTERS_cmdHandler(FwOpcodeType opCode, U32 cmdSeq) {
    HkTelemetryPkt.CommandCount = 0;
    HkTelemetryPkt.CommandErrorCount = 0;
    HkTelemetryPkt.DeviceCount = 0;
    HkTelemetryPkt.DeviceErrorCount = 0;
  ...
  void Payload_ifSim :: CONFIGURE_cmdHandler(FwOpcodeType opCode, U32 cmdSeq, const U32 config){
  ...
  inline Payload_ifSim_ActiveState Payload_ifSim :: get_active_state(uint8_t DeviceEnabled)
  {
    Payload_ifSim_ActiveState state;

    if(DeviceEnabled == PAYLOAD_IF_DEVICE_ENABLED)
    {
      state.e = Payload_ifSim_ActiveState::ENABLED;
    }
    else
    {
      state.e = Payload_ifSim_ActiveState::DISABLED;
    }

    return state;
  }

  //  void Payload_ifSim :: PAYLOAD_IF_SEQ_CANCEL_cmdHandler(FwOpcodeType opCode, U32 cmdSeq) {
