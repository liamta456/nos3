...
namespace Nos3
{
    ...
    _enabled(PAYLOAD_IF_SIM_SUCCESS), _count(0), _config(0), _status(0)
    {
        ...
        /* Get a data provider */
        std::string dp_name = config.get("simulator.hardware-model.data-provider.type", "PAYLOAD_IF_PROVIDER");
        _payload_if_dp = SimDataProviderFactory::Instance().Create(dp_name, config);
        sim_logger->info("Payload_ifHardwareModel::Payload_ifHardwareModel:  Data provider %s created.", dp_name.c_str());
        ...
        sim_logger->info("Payload_ifHardwareModel::Payload_ifHardwareModel:  Now on UART bus name %s, port %d.", bus_name.c_str(), node_port);
        ...
        sim_logger->info("Payload_ifHardwareModel::Payload_ifHardwareModel:  Construction complete.");
    }
    ...
    {        
        ...
        delete _payload_if_dp;
        _payload_if_dp = nullptr;
    ...
    void Payload_ifHardwareModel::command_callback(NosEngine::Common::Message msg)
    {
        ...
        std::string response = "Payload_ifHardwareModel::command_callback:  INVALID COMMAND! (Try HELP)";
        boost::to_upper(command);
        if (command.compare("HELP") == 0) 
        {
            response = "Payload_ifHardwareModel::command_callback: Valid commands are HELP, ENABLE, DISABLE, STATUS=X, or STOP";
        }
        else if (command.compare(0,6,"ENABLE") == 0) 
        {
            _enabled = PAYLOAD_IF_SIM_SUCCESS;
            response = "Payload_ifHardwareModel::command_callback:  Enabled\n";
        }
        else if (command.compare(0,7,"DISABLE") == 0) 
        {
            _enabled = PAYLOAD_IF_SIM_ERROR;
            _count = 0;
            _config = 0;
            _status = 0;
            response = "Payload_ifHardwareModel::command_callback:  Disabled";
        }
        else if (command.substr(0,7).compare("STATUS=") == 0)
        {
            try
            {
                ...
                response = "Payload_ifHardwareModel::command_callback:  Status set";
            }
            catch (...)
            {
                response = "Payload_ifHardwareModel::command_callback:  Status invalid";
            }            
        }
        else if (command.compare(0,4,"STOP") == 0) 
        {
            _keep_running = false;
            response = "Payload_ifHardwareModel::command_callback:  Stopping";
        }
        /* TODO: Add anything additional commands here */

        /* Send a reply */
        sim_logger->info("Payload_ifHardwareModel::command_callback:  Sending reply: %s", response.c_str());
        _command_node->send_reply_message_async(msg, response.size(), response.c_str());
    }


    /* Custom function to prepare the Payload_if HK telemetry */
    void Payload_ifHardwareModel::create_payload_if_hk(std::vector<uint8_t>& out_data)
    {
        /* Prepare data size */
    ...
    /* Custom function to prepare the Payload_if Data */
    void Payload_ifHardwareModel::create_payload_if_data(std::vector<uint8_t>& out_data)
    {
        boost::shared_ptr<Payload_ifDataPoint> data_point = boost::dynamic_pointer_cast<Payload_ifDataPoint>(_payload_if_dp->get_data_point());
        ...
        */
        double dx = data_point->get_payload_if_data_x();
        double dy = data_point->get_payload_if_data_y();
        double dz = data_point->get_payload_if_data_z();
        ...
        sim_logger->debug("Payload_ifHardwareModel::create_payload_if_data: data_point=%f, %f, %f, converted values=%u, %u, %u.", dx, dy, dz, x, y, z);
    ...
    void Payload_ifHardwareModel::uart_read_callback(const uint8_t *buf, size_t len)
    {
        ...
        std::uint8_t valid = PAYLOAD_IF_SIM_SUCCESS;
        
        /* Retrieve data and log in man readable format */
        std::vector<uint8_t> in_data(buf, buf + len);
        sim_logger->debug("Payload_ifHardwareModel::uart_read_callback:  REQUEST %s",
            SimIHardwareModel::uint8_vector_to_hex_string(in_data).c_str());

        /* Check simulator is enabled */
        if (_enabled != PAYLOAD_IF_SIM_SUCCESS)
        {
            sim_logger->debug("Payload_ifHardwareModel::uart_read_callback:  Payload_if sim disabled!");
            valid = PAYLOAD_IF_SIM_ERROR;
        }
        else
        {
            ...
            if (in_data.size() != 9)
            {
                sim_logger->debug("Payload_ifHardwareModel::uart_read_callback:  Invalid command size of %ld received!", in_data.size());
                valid = PAYLOAD_IF_SIM_ERROR;
            }
            else
            {
                ...
                {
                    sim_logger->debug("Payload_ifHardwareModel::uart_read_callback:  Header incorrect!");
                    valid = PAYLOAD_IF_SIM_ERROR;
                }
                else
                {
                    ...
                    {
                        sim_logger->debug("Payload_ifHardwareModel::uart_read_callback:  Trailer incorrect!");
                        valid = PAYLOAD_IF_SIM_ERROR;
                    }
                    else
                    {
            ...
            if (valid == PAYLOAD_IF_SIM_SUCCESS)
            {   
                /* Process command */
                switch (in_data[2])
                {
                    case 0:
                        /* NOOP */
                        sim_logger->debug("Payload_ifHardwareModel::uart_read_callback:  NOOP command received!");
                        break;

                case 1:
                        /* Request HK */
                        sim_logger->debug("Payload_ifHardwareModel::uart_read_callback:  Send HK command received!");
                        create_payload_if_hk(out_data);
                        break;

                    case 2:
                        /* Request data */
                        sim_logger->debug("Payload_ifHardwareModel::uart_read_callback:  Send data command received!");
                        create_payload_if_data(out_data);
                        break;

                    case 3:
                        /* Configuration */
                        sim_logger->debug("Payload_ifHardwareModel::uart_read_callback:  Configuration command received!");
                    ...
                    default:
                        /* Unused command code */
                        valid = PAYLOAD_IF_SIM_ERROR;
                        sim_logger->debug("Payload_ifHardwareModel::uart_read_callback:  Unused command %d received!", in_data[2]);
                        break;
                }
            }
        }
        ...
        if (valid == PAYLOAD_IF_SIM_SUCCESS)
        {
            ...
            if (out_data.size() > 0)
            {
                sim_logger->debug("Payload_ifHardwareModel::uart_read_callback:  REPLY %s",
