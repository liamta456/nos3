...
## Raspberry Pi
##
# gnome-terminal --tab --title="Raspberry Pi Sim" -- $DFLAGS -v $SIM_DIR:$SIM_DIR --name $SC_NUM"_raspberry_pi_sim" --network=$SC_NETNAME -w $SIM_BIN $DBOX ./nos3-single-simulator $SC_CFG_FILE raspberry-pi-sim
# gnome-terminal --title="Raspberry Pi Checkout" -- $DFLAGS -v $BASE_DIR:$BASE_DIR --name $SC_NUM"_raspberry_pi_checkout" --network=$SC_NETNAME -w $BASE_DIR $DBOX ./components/raspberry_pi/fsw/standalone/build/raspberry_pi_checkout
